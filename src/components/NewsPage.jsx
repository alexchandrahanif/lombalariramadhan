...
const NewsPage = ({ slug }) => {
	...
	return <div className="match-page">
		...
		<main className="article-page"><div className="article-hero"><span>{article.category}</span><h1>{article.title}</h1><p>{article.excerpt}</p><small><CalendarDays /> {article.date}</small></div><img className="article-cover" src={article.image} alt={article.title} decoding="async" /><article className="article-body">{article.paragraphs.map((paragraph, paragraphIndex) => <div key={paragraph}>{paragraphIndex === 1 && article.qualifiedTeams && <div className="qualified-table-wrap"><table className="qualified-table"><thead><tr><th>Group</th><th>Posisi</th><th>Tim Lolos</th></tr></thead><tbody>{article.qualifiedTeams.map((team) => <tr key={`${team.group}-${team.position}`}><td>{team.group}</td><td>#{team.position}</td><td>{team.name}</td></tr>)}</tbody></table></div>}<p>{paragraph}</p></div>)}<div className="article-share"><Share2 /><div><strong>Bagikan berita pertandingan</strong><small>Salin URL halaman ini, lalu kirim melalui WhatsApp atau media sosial.</small></div></div></article><aside className="article-related"><span>Berita Lainnya</span>{news.filter((item) => item.slug !== slug).slice(0,5).map((item) => <a href={`/berita/${item.slug}`} key={item.slug}><img src={item.image} alt="" loading="lazy" decoding="async" /><div><small>{item.date}</small><strong>{item.title}</strong></div><ArrowRight /></a>)}<a className="article-related__more" href="/berita"><span>Lihat Selengkapnya</span><ArrowRight /></a></aside></main>
	</div>;
};
