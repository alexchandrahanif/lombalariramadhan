...
const MatchCenterPage = () => {
	...
	return <div className="match-page">
		<header className="match-nav"><a href="/" className="match-brand"><img src={eventLogo} alt="" /><span><strong>Pemuda Cup III</strong><small>Match Center</small></span></a><nav><a href="#pertandingan">Pertandingan</a><a href="/8-besar">8 Besar</a><a href="/berita">Berita</a></nav><a href="/group" className="match-nav__back"><ArrowLeft /> Group</a></header>
		<main>
			<section className="match-hero"><div><span className="match-eyebrow"><Trophy /> Pusat Pertandingan</span><h1>Match<br/><em>Center.</em></h1><p>Jadwal, hasil pertandingan, statistik pemain, dan kabar terbaru Pemuda Cup III dalam satu tempat.</p><div className="match-hero__meta"><span><MapPin /> Lapangan Putri Hijau</span><span><CalendarDays /> 2–20 September 2026</span></div></div><div className="match-hero__logo" aria-label="Pemuda Cup III Sintong 2026"><span>Turnamen Mini Soccer</span><img src={eventLogo} alt="Logo Pemuda Cup III Sintong" /><strong>Pemuda Cup III</strong><small>Sintong · 2026</small></div></section>
			<section className="match-section" id="pertandingan">
				...
				<div className="match-date-filter"><label htmlFor="match-date"><CalendarDays /> Pilih Tanggal</label><input id="match-date" type="date" value={selectedDate} min="2026-09-02" max="2026-09-20" onChange={(event) => setSelectedDate(event.target.value)} /><strong>{dateLabel(selectedDate)}</strong></div>
